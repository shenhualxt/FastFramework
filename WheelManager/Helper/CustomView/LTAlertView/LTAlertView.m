...
- (void)showCustomAlert {
    ...
    CGSize customViewSize= [self.customView systemLayoutSizeFittingSize:UILayoutFittingCompressedSize];
    if (!self.customView.constraints.count) {
        customViewSize=self.customView.frame.size;
    }
    self.edgeTopConstraint = [self autoPinEdgeToSuperviewEdge:ALEdgeTop withInset:-customViewSize.height];
    if (customViewSize.width>kAlertWidth) {
        customViewSize.width=kAlertWidth;
    }
    [self autoPinEdgeToSuperviewEdge:ALEdgeLeft withInset:([UIScreen mainScreen].bounds.size.width - customViewSize.width) * 0.5];
    [self autoSetDimension:ALDimensionWidth toSize:customViewSize.width];
    [self autoSetDimension:ALDimensionHeight toSize:customViewSize.height];

    //动画 从顶部移动到中间
    [self animateWithY:([UIScreen mainScreen].bounds.size.height - customViewSize.height) * 0.5 removeFromSuperView:NO];
}
